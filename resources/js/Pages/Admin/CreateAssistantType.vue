<template>
    <AdminLayout :auth="auth">
        <div class="max-w-2xl mx-auto">
            <div class="mb-6">
                <h2 class="text-2xl font-semibold text-gray-900">Tạo Loại Trợ Lý Mới</h2>
                <p class="text-gray-600 mt-2">Tạo loại trợ lý mới để sử dụng khi tạo assistant</p>
            </div>

            <form @submit.prevent="createAssistantType" class="bg-white rounded-lg shadow-md p-6 space-y-6">
                <!-- Name -->
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">
                        Mã loại sẽ được tự động tạo từ tên (ví dụ: "Trả lời Q&A từ tài liệu" → "tra_loi_qa_tu_tai_lieu")
                    </p>
                ...
                <div v-if="form.code" class="bg-gray-50 border border-gray-200 rounded-lg p-3">
                ...
                <div class="flex justify-end space-x-3 pt-4">
                    ...
                    >
                        ...
                        <span v-else>Tạo Loại Trợ Lý</span>
                    </button>
                </div>
            </form>
        </div>
...
// Convert Vietnamese text to slug/code
const generateCodeFromName = () => {
    ...
    let code = form.name
        ...
        .replace(/đ/g, 'd')
        .replace(/[^a-z0-9\s-]/g, '')
        // Replace spaces and multiple dashes with underscore
        .replace(/\s+/g, '_')
        .replace(/-+/g, '_')
        .replace(/_+/g, '_')
...
const createAssistantType = async () => {
    isSubmitting.value = true;
    errorMessage.value = '';
    successMessage.value = '';

    try {
        const response = await axios.post('/admin/assistant-types', form);
        
        successMessage.value = response.data.message || 'Loại trợ lý đã được tạo thành công!';
        ...
        console.error('Error creating assistant type:', error);
        
        if (error.response?.data?.errors) {
            // Validation errors sẽ được hiển thị qua props.errors
            errorMessage.value = 'Vui lòng kiểm tra lại các trường bên dưới.';
        } else {
            errorMessage.value = error.response?.data?.message || 'Không thể tạo loại trợ lý. Vui lòng thử lại.';
        }
    } finally {
