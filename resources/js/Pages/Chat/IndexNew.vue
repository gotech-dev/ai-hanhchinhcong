                    ...
                    >
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <div class="flex gap-2">
                                <!-- ✅ Button Sửa/Thoát với loading -->
                                <button
                                    v-if="message.metadata?.template_html"
                                    @click="toggleTemplateEditMode(message.id)"
                                    :disabled="templateSavingStates[message.id]"
                                    class="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600 text-sm font-medium flex items-center gap-2 transition-colors shadow-sm hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
                                    :title="templateEditModes[message.id] ? 'Thoát chế độ chỉnh sửa' : 'Chỉnh sửa trực tiếp'"
                                >
                                    <!-- Loading spinner -->
                                    <svg v-if="templateSavingStates[message.id]" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
